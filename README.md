...
6、如需将项目打包成jar包，需先打包前端页面，具体查看**悦听影音后台管理系统前端界面**（[**github**](https://github.com/LMINediva/melodious-player-admin) | [**gitee**](https://gitee.com/lminediva/melodious-player-admin)）的运行说明，然后运行IDEA中的Gradle -> Tasks -> build -> bootJar 进行打包，打包好后，需将演示资源文件解压到jar包所在目录中，然后运行：java -Djasypt.encryptor.password=你自己的jasypt加密的秘钥 -jar xxx.jar 命令来运行jar包。